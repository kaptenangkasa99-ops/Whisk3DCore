...
#include "crossplatform.h" // compat (engine, resuelto por el include dir engine)
...
struct Quaternion {
    // sin inicializadores en la clase (C++11): el constructor de abajo ya
    // inicializa todo; asi compila en RVCT 2.2 (C++03) y en PC por igual
    float w;
    float x;
    float y;
    float z;
    ...
    // inverso exacto (consistencia para editar el numero).
    static Quaternion FromEulerXYZ(float xDeg, float yDeg, float zDeg);
    Vector3 ToEulerXYZ() const;
    // Axis-Angle: angulo (grados) + eje normalizado. FromAxisAngle ya existe.
    void ToAxisAngle(float& angleDeg, Vector3& axis) const;
